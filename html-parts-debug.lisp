...
(defun print-tree (&key (indent 0) (children (list (weblocks-util:get-html-parts-root-hash))))
    ...
    (loop for i in (reverse children) do 
          ...
          (with-html 
            (:a :href (ps:ps-inline (show-element-rect (get-dom-by-hash (ps:LISP i))))
             (cl-who:esc (format nil "~A" (truncate-string (weblocks-util:get-html-part i) :length 20))))
            (str " ")
            (let ((i-copy i))
              (render-link 
                (lambda (&rest args)
                                     ...
                                     (str (pretty-print-to-html 
                                            (arnesi:escape-as-html 
                                              (with-output-to-string (s)
                                                (pprint (weblocks-util:get-html-part-context i) s)))))
                                     (:h1 "Element text")
                                     ...
                                     (str (pretty-print-to-html 
                                            (arnesi:escape-as-html 
                                              (or (cl-tidy:clean-up-and-indent-html-part 
                                                    (weblocks-util:get-html-part i)) ""))))
                                     (:br)
                                     (render-link back-action "back")))))))
                "info"))
            (let ((type (getf (weblocks-util:get-html-part-context i) :type)))
              (when (and type (equal type :template))
                (html-newline-and-indent)
                (cl-who:htm 
                  (:i :style "text-align:right;"
                   (str (string-downcase (prin1-to-string (getf (weblocks-util:get-html-part-context i) :template-name)))))))))
          (print-tree :indent (+ indent 2)
            :children (weblocks-util:get-html-part-children i)))))
      ...
      (setf document-data 
            (eval 
              (ps:LISP 
                (format 
                  nil "(~A)"
                  (cl-json:encode-json-to-string weblocks-util::*parts-md5-hash*)))))))
  (send-script 
    (ps:ps 
